...
def _check_gaussian_weights(discretized_gaussian):
    total = sum(entry["weight"] for entry in discretized_gaussian)
    if abs(total - 1.0) > 1e-9:
        print(
            f"[warn] config.yaml's noise.discretized_gaussian weights sum to "
            f"{total!r}, not 1.0 -- ProbLog silently treats the missing mass "
            f"as an implicit failure branch, which will cap every downstream "
            f"probability. Fix the weights in config.yaml.",
            file=sys.stderr,
        )
...
def render_prolog(config):
    _check_gaussian_weights(config["noise"]["discretized_gaussian"])
    _check_sight_threshold(config)

    triggers_list = "[" + ",".join(config["triggers"]["default"]) + "]"

    z_block = _gaussian_disjunction(
        "z", "do(startMoveto(CP,Triggers,T0),S), ",
        config["noise"]["discretized_gaussian"])
    zbatt_block = _gaussian_disjunction(
        "zbatt", "", config["noise"]["discretized_gaussian"])
    ...
    lines = [
        ...
        f"default_triggers({triggers_list}).",
        f"speed({_format_number(config['motion']['speed'])}).",
        f"sigma({_format_number(config['noise']['position_sigma'])}).",
        f"sigma_battery({_format_number(config['noise']['battery_sigma'])}).",
